...
// SHT31 only enabled when needed with D7
#define SHT31_PWR_PIN 7
...
void enableSensor(const bool enable)
{
	if(enable)
	{
		pinMode(SHT31_PWR_PIN, OUTPUT);
		digitalWrite(SHT31_PWR_PIN, HIGH);
	}
	else
	{
		pinMode(SHT31_PWR_PIN, OUTPUT);
		digitalWrite(SHT31_PWR_PIN, LOW);
	}
}
